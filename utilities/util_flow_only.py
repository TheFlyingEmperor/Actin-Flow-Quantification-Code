...
def plot_flow_on_paxillin(x, y, u, v, im, pax_im, actin, frame,
                          nm_per_pix, time_btwn_frames, norm, scale=80):
    """
    ...
    ax1.imshow(pax_im, cmap='gray', norm=norm)
    ax1.quiver(x, y, mask_u, -mask_v, scale=scale, width=0.003, color='red')
    ax1.set_title(f'Paxillin with Actin Flow (Frame {frame})')
    ...
    ax2.quiver(x, im.shape[0] - y, mask_u, -mask_v, scale=scale, width=0.003)
    fig.colorbar(magnitude, fraction=0.025, pad=0.01, ax=ax2)
    ax2.set_title('Flow Speed (nm/s)')
